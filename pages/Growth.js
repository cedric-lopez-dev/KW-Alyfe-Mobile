...
import CardGrowth from '../Components/CardGrowth';
...
const Growth = ({ navigation }) => {
    ...
    const handleReco = () => {
        data.agent.mcLeadGen &&
            navigation.navigate('Recommandation')
    }

    const handleBoost = async () => {
        if (data.agent.mcLeadGen)
            try {
...

            {
                ...
                <>
                    <View style={styles.container}>
                        ...
                        <View style={[styles.buttonContainer, !data?.agent?.mcLeadGen && styles.buttonNoLeadgen]}>
                            ...
                            <ScrollView
...

                                {
                                    data?.leads && data?.leads
                                        ...
                                        .map((lead, i) => (
                                            <CardGrowth key={lead.id} lead={lead} last={i === data.leads.length - 1} />
                                        ))
                                }
                            </ScrollView> :
