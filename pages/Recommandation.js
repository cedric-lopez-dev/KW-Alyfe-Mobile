...
const Recommandation = ({ navigation }) => {
    ...
    const handleSubitForm = () => {
        ...
        if (errors.phone || errors.fullname || errors.reco) {
            ...
            const lead =
                { ...formData?.lead, phone: formattedValue, origin: { type: 'member', name: 'recommandation', memberId: data.agent.id, mobile: true } }
            createLead(lead)
                .then(() => {
    ...
    const handleTeamLeaderCard = (teamLeader) => {
        ...
        setFormData({ ...formData, teamLeader: { firstname: teamLeader.firstname, lastname: teamLeader.lastname }, lead: { ...formData?.lead, affectedTo: teamLeader.id, mcId: teamLeader?.marketCenter.id } })
    }
